...
from ecdsa import generate_k
from transform import sha256_twice
...
def generate_keypair(curve, P, n):
    ...
    Q = curve.mult(P, d)
    print("Priv key: d = " + hex(d))
    print("Publ key: Q = " + hex(Q.x) + ', ' + hex(Q.y))
    return (d, Q)
...
def sign(message, curve, P, n, keypair):
    ...
    while r == 0 or s == 0:
        ...
        # k = generate_k(sha256_twice(message.encode('utf-8')), hex(keypair[0])[2:])
        # print(sha256_twice(message.encode('utf-8')))
        R = curve.mult(P, k)
        # print(R)
        r = R.x % n
