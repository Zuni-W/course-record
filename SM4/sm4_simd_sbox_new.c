 ...
 */
static void _SM4_do(uint32_t* in, uint32_t* out, SM4_Key sm4_key, int enc);

void SM4_Encrypt_x8(uint32_t* in, uint32_t* out, SM4_Key sm4_key) {
    _SM4_do(in, out, sm4_key, 0);
}
void SM4_Decrypt_x8(uint32_t* in, uint32_t* out, SM4_Key sm4_key) {
...
void _SM4_do(uint32_t* in, uint32_t* out, SM4_Key sm4_key, int enc) {
    register __m256i X1,X2,X3,X4, T1,T2,T3,T4, Mask;
    Mask = _mm256_set1_epi32(0xFF);
    //加载数据
    T1 = _mm256_loadu_si256((const __m256i*)in + 0);
    T2 = _mm256_loadu_si256((const __m256i*)in + 1);
    T3 = _mm256_loadu_si256((const __m256i*)in + 2);
    T4 = _mm256_loadu_si256((const __m256i*)in + 3);
    //合并每组128bit数据的某32bit字
    X1 = MM256_PACK0_EPI32( T1, T2, T3,T4);
    X2 = MM256_PACK1_EPI32( T1, T2, T3,T4);
    X3 = MM256_PACK2_EPI32( T1, T2, T3,T4);
    X4 = MM256_PACK3_EPI32( T1, T2, T3,T4);
    //转化端序
    for (int i = 0; i < 32; i+=4) {
        register __m256i k =
            _mm256_set1_epi32((enc == 0) ? sm4_key[i] : sm4_key[31 - i]);
        T1 = _mm256_xor_si256(_mm256_xor_si256(X2, X3),
                                   _mm256_xor_si256(X4, k));
        //查表
        T2 = _mm256_xor_si256(
            X1, _mm256_i32gather_epi32((const int*)BOX0,
                                         _mm256_and_si256(T1, Mask), 4));
        T1 = _mm256_srli_epi32(T1, 8);
        T2 = _mm256_xor_si256(
            T2, _mm256_i32gather_epi32(
                         (const int*)BOX1, _mm256_and_si256(T1, Mask), 4));
        T1 = _mm256_srli_epi32(T1, 8);
        T2 = _mm256_xor_si256(
            T2, _mm256_i32gather_epi32(
                         (const int*)BOX2, _mm256_and_si256(T1, Mask), 4));
        T1 = _mm256_srli_epi32(T1, 8);
        T2 = _mm256_xor_si256(
            T2, _mm256_i32gather_epi32(
                         (const int*)BOX3, _mm256_and_si256(T1, Mask), 4));
        X1=T2;

        k=    _mm256_set1_epi32((enc == 0) ? sm4_key[i+1] : sm4_key[30 - i]);
        T1 = _mm256_xor_si256(_mm256_xor_si256(X1, X3),
                                   _mm256_xor_si256(X4, k));
        //查表
        T2 = _mm256_xor_si256(
            X2, _mm256_i32gather_epi32((const int*)BOX0,
                                         _mm256_and_si256(T1, Mask), 4));
        T1 = _mm256_srli_epi32(T1, 8);
        T2 = _mm256_xor_si256(
            T2, _mm256_i32gather_epi32(
        ...
        k=    _mm256_set1_epi32((enc == 0) ? sm4_key[i+2] : sm4_key[29 - i]);
        T1 = _mm256_xor_si256(_mm256_xor_si256(X1, X2),
                                   _mm256_xor_si256(X4, k));
        //查表
        T2 = _mm256_xor_si256(
            X3, _mm256_i32gather_epi32((const int*)BOX0,
                                         _mm256_and_si256(T1, Mask), 4));
        T1 = _mm256_srli_epi32(T1, 8);
        T2 = _mm256_xor_si256(
            T2, _mm256_i32gather_epi32(
        ...
        k=    _mm256_set1_epi32((enc == 0) ? sm4_key[i+3] : sm4_key[28 - i]);
        T1 = _mm256_xor_si256(_mm256_xor_si256(X1, X3),
                                   _mm256_xor_si256(X2, k));
        //查表
        T2 = _mm256_xor_si256(
            X4, _mm256_i32gather_epi32((const int*)BOX0,
                                         _mm256_and_si256(T1, Mask), 4));
        T1 = _mm256_srli_epi32(T1, 8);
        T2 = _mm256_xor_si256(
            T2, _mm256_i32gather_epi32(
    ...
    _mm256_storeu_si256((__m256i*)out + 0,
                        MM256_PACK0_EPI32(X4, X3, X2, X1));
    _mm256_storeu_si256((__m256i*)out + 1,
                        MM256_PACK1_EPI32(X4, X3, X2, X1));
    _mm256_storeu_si256((__m256i*)out + 2,
                        MM256_PACK2_EPI32(X4, X3, X2, X1));
    _mm256_storeu_si256((__m256i*)out + 3,
                        MM256_PACK3_EPI32(X4, X3, X2, X1));
}
